...
@IBDesignable class GradientSlider: UIControl {
    ...
    @IBInspectable var midColor:UIColor = UIColor.orange {didSet{updateTrackColors()}}
    ...
    @IBInspectable var minimumValueImage: UIImage? = nil { // default is nil. image that appears to left of control (e.g. speaker off)
        didSet{
            if let img = minimumValueImage {
                ...
                imgLayer.bounds = CGRect.init(x: 0, y: 0, width: img.size.width, height: img.size.height)
    ...
    var thumbSize:CGFloat = defaultThumbSize {
        didSet{
            ...
            _thumbLayer.bounds = CGRect.init(x: 0, y: 0, width: thumbSize, height: thumbSize)
    ...
    private var _thumbLayer:CALayer = {
        ...
        thumb.bounds = CGRect.init(x: 0, y: 0, width: defaultThumbSize, height: defaultThumbSize)
        thumb.backgroundColor = UIColor.white.cgColor
        thumb.shadowColor = UIColor.black.cgColor
        thumb.shadowOffset = CGSize.init(width: 0.0, height: 2.5)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        midColor = aDecoder.decodeObject(forKey: "midColor") as? UIColor ?? UIColor.lightGray
    ...
    override func encode(with aCoder: NSCoder) {
        ...
        aCoder.encode(midColor, forKey: "midColor")
...
//        super.layoutSublayersOfLayer(layer)
    ...
    private func updateTrackColors() {
        if !hasRainbow {
            _trackLayer.colors = [minColor.cgColor, midColor.cgColor, maxColor.cgColor]
            _trackLayer.locations = [0.0, 0.5, 1.0]
            return
        }
